...
public class Converter {
    ...
    public static void printFahrenheitConversionTable(int lower, int upper) {
        ...
        for (i = lower; i <= upper; i++) {
            convert = (i - 32) * (5.0 / 9);
            System.out.printf("Fahrenheit: %d -> Celsius: %.3f\n", i, convert);
        }
    ...
    public static void printCelsiusConversionTable(int lower, int upper) {
        double convert;
        while (lower <= upper) {
            convert = lower * (9.0 / 5) + 32;
            System.out.printf("Celsius: %d -> Fahrenheit: %.3f\n", lower, convert);
            lower++;
        }
    }
     ...
     * @param celsius the second temperature in degrees celsiu
s     * @return true iff the first temperature is bigger.
     */
    public static boolean fahrenheitWarmer(int fahrenheit, int celsius) {
        double convert;
        convert = (fahrenheit - 32) * (5.0 / 9);
        boolean result;
        result = convert > celsius ? true : false;
        return result;
    }
}
